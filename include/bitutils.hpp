...
inline __m128i xor_if(const __m128i &block1, const __m128i &block2,
                      __m128i flag) {
...
inline std::array<__m128i, LWIDTH>
xor_if(const std::array<__m128i, LWIDTH> &block1,
...
inline uint8_t get_lsb(const __m128i &block, uint8_t bits = 0b01) {
  __m128i vcmp =
      _mm_xor_si128(_mm_and_si128(block, lsb128_mask[bits]), lsb128_mask[bits]);
...
inline uint8_t parity_above(const __m128i &block, uint8_t position) {
  ...
  if (position >= 64) {
    ...
    mask |= (mask - 1);
    mask = ~mask;
    return ((__builtin_popcountll(high) + __builtin_popcountll(low & mask)) &
            1);
  }
}
...
inline uint8_t parity_below(const __m128i &block, uint8_t position) {
  ...
  if (position >= 64) {
    ...
    return ((__builtin_popcountll(low) + __builtin_popcountll(high & mask)) &
            1);
  } else {

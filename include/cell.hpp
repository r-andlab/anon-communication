...
#define CELL_VALUE field(&Cell::value)
  ...
  inline static void update(Sh &shape, yield_t &shyield, U idx, const Cell &M) {
    run_coroutines(
        shyield,
...
template <typename T> T &operator>>(T &is, Cell &x) {
  is >> x.key >> x.pointers >> x.value;
  return is;
}

template <typename T> T &operator<<(T &os, const Cell &x) {
...
void cell(MPCIO &mpcio, const PRACOptions &opts, char **args);

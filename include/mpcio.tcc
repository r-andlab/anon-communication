...
template <typename T, typename N> void PreCompStorage<T, N>::get(T &nextval) {
  storage >> nextval;
  if (!storage.good()) {
...
RDPFTriple<WIDTH> MPCTIO::rdpftriple(yield_t &yield, nbits_t depth,
  ...
  if (mpcio.mode == MODE_ONLINE) {
    if (incremental) {
      std::get<WIDTH - 1>(mpcpio.irdpftriples)[thread_num][depth - 1].get(val);
    } else {
      std::get<WIDTH - 1>(mpcpio.rdpftriples)[thread_num][depth - 1].get(val);
    }
  } else {
    val = RDPFTriple<WIDTH>(*this, yield, depth, incremental, keep_expansion);
    iostream_server() << val.dpf[(mpcio.player == 0) ? 1 : 2];
    if (incremental) {
      std::get<WIDTH - 1>(mpcpio.irdpftriples)[thread_num][depth - 1].inc();
    } else {
...
RDPFPair<WIDTH> MPCTIO::rdpfpair(yield_t &yield, nbits_t depth,
  ...
  if (mpcio.mode == MODE_ONLINE) {
    if (incremental) {
      std::get<WIDTH - 1>(mpcsrvio.irdpfpairs)[thread_num][depth - 1].get(val);
    } else {
    ...
    if (incremental) {
      std::get<WIDTH - 1>(mpcsrvio.irdpfpairs)[thread_num][depth - 1].inc();
    } else {

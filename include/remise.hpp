...
private:
  ...
  std::pair<RegXS, RegBS> restore_heap_property_optimized(
  ...
  restore_heap_property_at_explicit_index(MPCTIO &tio, yield_t &yield,
                                          size_t index);

public:
  RemiseSAM(int player_num, size_t size)
      : oram(player_num, size), tokendb(player_num, size),
        proofdb(player_num, size) {};

  void initialize_proofDB(MPCTIO &tio, yield_t &yield, size_t dbsize,
                          Writer writer);

  void post_message(MPCTIO &tio, yield_t &yield, RegAS ind, RegAS val,
                    RegAS proof);

  RegAS read_message(MPCTIO &tio, yield_t &yield, RegAS ind,
                     RegAS proof_submitted);

  void provide_or_remove_access(MPCTIO &tio, yield_t &yield, RegAS ind,
                                RegAS proof_submitted);

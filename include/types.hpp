...
#define MASKBITS(nbits)                                                        \
  (((nbits) < VALUE_BITS) ? (value_t(1) << (nbits)) - 1 : ~0)
...
struct RegAS {
  ...
  inline void dump() const { printf("%016lx", ashare); }
};
...
struct RegXS {
  ...
  inline void dump() const { printf("%016lx", xshare); }
...
template <typename T> struct prac_basic_Reg_S : prac_template_false {
  static const bool value = false;
};
template <> struct prac_basic_Reg_S<RegAS> : prac_template_true {
  static const bool value = true;
};
template <> struct prac_basic_Reg_S<RegXS> : prac_template_true {
  static const bool value = true;
};
...
template <typename T>
std::tuple<T, T> operator+=(std::tuple<T, T> &A, const std::tuple<T, T> &B) {
...
std::tuple<std::array<T, N>, std::array<T, N>>
operator*(const std::tuple<T, T> &A,
...
combine(const std::tuple<std::array<S, N>, std::array<S, N>> &A,
        ...
        nbits_t nbits = VALUE_BITS) {
  return std::make_tuple(combine(std::get<0>(A), std::get<0>(B), nbits),
...
template <typename T, size_t N>
std::tuple<std::array<T, N>, std::array<T, N>, std::array<T, N>>
operator*(const std::tuple<T, T, T> &A,
          const std::tuple<std::array<value_t, N>, std::array<value_t, N>,
                           std::array<value_t, N>> &B) {
...
inline std::vector<RegXS> operator-(const std::vector<RegXS> &A) { return A; }

inline std::vector<RegBS> operator-(const std::vector<RegBS> &A) { return A; }
...
template <typename S, size_t N>
inline std::array<S, N> &operator+=(std::array<S, N> &A,
                                    const std::array<S, N> &B) {
  for (size_t i = 0; i < N; ++i) {
    A[i] += B[i];
  }
...
template <typename V> struct SelectTriple {
  bit_t X;
  V Y, Z;
};
// Of the three options for V, we only ever store V = value_t
struct ValSelectTripleName {
...
#define DEFAULT_IO(CLASSNAME)                                                  \
  template <typename T> T &operator>>(T &is, CLASSNAME &x) {                   \
    is.read((char *)&x, sizeof(x));                                            \
    return is;                                                                 \
  }                                                                            \
                                                                               \
  template <typename T> T &operator<<(T &os, const CLASSNAME &x) {             \
...
template <typename T, typename V> T &operator>>(T &is, SelectTriple<V> &x) {
...
T &operator>>(
    T &is,
    std::tuple<std::array<S, N>, std::array<S, N>, std::array<S, N>> &x) {
  is >> std::get<0>(x) >> std::get<1>(x) >> std::get<2>(x);
  return is;
}
...
T &operator<<(
    T &os,
    const std::tuple<std::array<S, N>, std::array<S, N>, std::array<S, N>> &x) {
  os << std::get<0>(x) << std::get<1>(x) << std::get<2>(x);
  return os;
}

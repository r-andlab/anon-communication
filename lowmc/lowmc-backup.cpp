...
class lowmc
{
  public:
	...
	//using inputsocketstream = streams::input_socket_stream<block_t, 1>;
...
/** @} */ // end of group2
 ...
 //    block_t maska = block_t(std::string(sbox_mask, 0, identity_len  - 1));
	...
	// inline auto mul2(const uint64_t matrix[block_len*sizeof(block_t)/sizeof(uint64_t)/4][16],
	// 	const block_t & msg, const block_t & constant) const
	// {
	// 	auto result = constant;
	// 	const uint64_t * msg_bytes = reinterpret_cast<const uint64_t*>(&msg);
	// 	for (size_t i = 0; i < sizeof(block_t); ++i)
	// 	{
	// 		result ^= matrix[i][(msg_bytes[i] >>  0) & 0x0f];
	// 		result ^= matrix[i][(msg_bytes[i] >>  4) & 0x0f];
	// 		result ^= matrix[i][(msg_bytes[i] >>  8) & 0x0f];
	// 		result ^= matrix[i][(msg_bytes[i] >> 12) & 0x0f];
	// 		result ^= matrix[i][(msg_bytes[i] >> 16) & 0x0f];
	// 		result ^= matrix[i][(msg_bytes[i] >> 20) & 0x0f];
	// 		result ^= matrix[i][(msg_bytes[i] >> 24) & 0x0f];
	// 		result ^= matrix[i][(msg_bytes[i] >> 28) & 0x0f];
	// 		result ^= matrix[i][(msg_bytes[i] >> 32) & 0x0f];
	// 		result ^= matrix[i][(msg_bytes[i] >> 36) & 0x0f];
	// 		result ^= matrix[i][(msg_bytes[i] >> 40) & 0x0f];
	// 		result ^= matrix[i][(msg_bytes[i] >> 44) & 0x0f];
	// 		result ^= matrix[i][(msg_bytes[i] >> 48) & 0x0f];
	// 		result ^= matrix[i][(msg_bytes[i] >> 52) & 0x0f];
	// 		result ^= matrix[i][(msg_bytes[i] >> 56) & 0x0f];
	// 		result ^= matrix[i][(msg_bytes[i] >> 60) & 0x0f];
	// 	}
	// 	return result;
	// } // mul2
...
__m128i _mm_set1_epi8_xor(__m128i message, bool x)
{
  if(x) return message ^ _mm_set1_epi8(-1);
	...
	// inline auto encrypt2_p0p1(const std::array<block<__m256i> , 128> & share0, randomness & rand0,
	// 	instream * p1in, outstream * p2, outstream * p1out, bool p) const
	inline auto encrypt2_p0p1(const std::array<block<__m256i> , 128> & share0, const std::array<block<__m256i> , 3 * sboxes> & blind0,  
							  const std::array<block<__m256i> , 3 * sboxes> & gamma, bool p) const
	{
		auto c0 = share0;
 		
 		std::array<block<__m256i> , 128> blinded_c1;
		...
		for (size_t i = 0; i < rounds; ++i)
		{
			...
			// *p1in >> blinded_c1;
			// *p2 >> gamma;
			c0 = substitute2_p0p1_(c0, blind0, blinded_c1, gamma);
			c0 = mmul(matrices[i], c0, p ? round_constants[i] : 0);
		}
		return c0;
	} // encrypt2_p0p1
...
_mm256_movemask_epi8(_mm256_slli_epi64(x, 7))
  ...
  //private:
	...
	inline auto & substitute2_p0p1_(  std::array<block<__m256i>, 128>& share0,  const std::array<block<__m256i>, 3 * sboxes>& blind0,
									 const std::array<block<__m256i>, 128>& blinded_share1, const std::array<block<__m256i>, 3 * sboxes>& gamma0) const 
	{
		for(size_t i = 0, j = 0; i < sboxes; ++i, j+=3)
		{

...
void print_red(std::string line) { printf("%s%s%s", KRED, line.c_str(), KNRM); }
...
void AVL::pretty_print(const std::vector<Node> &R, value_t node,
  ...
  std::string rightprefix(prefix), leftprefix(prefix), nodeprefix(prefix);
  if (is_left_child) {
...
*/
std::tuple<bool, bool, bool, address_t>
AVL::check_avl(const std::vector<Node> &R, value_t node, value_t min_key = 0,
               value_t max_key = ~0) {
  if (node == 0) {
  ...
  value_t left_ptr = getAVLLeftPtr(n.pointers).xshare;
  value_t right_ptr = getAVLRightPtr(n.pointers).xshare;
  auto [leftok, leftavlok, leftbbok, leftheight] =
      check_avl(R, left_ptr, min_key, key);
  auto [rightok, rightavlok, rightbbok, rightheight] =
      check_avl(R, right_ptr, key, max_key);
  ...
  return {leftok && rightok && key >= min_key && key <= max_key,
          avlok && leftavlok && rightavlok, bb_ok && leftbbok && rightbbok,
          height};
}
...
bool AVL::check_avl(MPCTIO &tio, yield_t &yield) {
  ...
  if (tio.player() == 0) {
    ...
    printf("BST structure %s\nAVL structure %s\nBalance Bits %s\nTree height = "
           "%u\n",
           bst_ok ? "ok" : "NOT OK", avl_ok ? "ok" : "NOT OK",
           bb_ok ? "ok" : "NOT OK", height);
...
void AVL::rotate(MPCTIO &tio, yield_t &yield, RegXS &gp_pointers, RegXS p_ptr,
                 RegXS &p_pointers, RegXS c_ptr, RegXS &c_pointers,
                 RegBS dir_gpp, RegBS dir_pc, RegBS isReal, RegBS F_gp) {
  ...
  coroutines.emplace_back([&tio, &F_gppr, F_gpp, dir_gpp](yield_t &yield) {
    mpc_and(tio, yield, F_gppr, F_gpp, dir_gpp);
  });
  coroutines.emplace_back([&tio, &F_gppl, F_gpp, not_dir_gpp](yield_t &yield) {
  ...
  coroutines.emplace_back([&tio, &F_pc_l, dir_pc, isReal](yield_t &yield) {
    mpc_and(tio, yield, F_pc_l, dir_pc, isReal);
  });
  coroutines.emplace_back([&tio, &F_pc_r, nt_dir_pc, isReal](yield_t &yield) {
  ...
  run_coroutines(
      tio,
      [&tio, &gp_right, F_gppr, ptr_upd](yield_t &yield) {
      ...
      [&tio, &c_not_dir_pc, F_ndpc_right, c_right](yield_t &yield) {
        mpc_select(tio, yield, c_not_dir_pc, F_ndpc_right, c_not_dir_pc,
                   c_right, AVL_PTR_SIZE);
      });

  //[&tio, &c_not_dir_pc, F_ndpc_left, c_left](yield_t &yield)
  mpc_select(tio, yield, c_not_dir_pc, F_ndpc_left, c_not_dir_pc, c_left,
             AVL_PTR_SIZE);
  ...
  run_coroutines(
      tio,
      [&tio, &p_left, F_ndpc_right, c_not_dir_pc](yield_t &yield) {
        mpc_select(tio, yield, p_left, F_ndpc_right, p_left, c_not_dir_pc,
                   AVL_PTR_SIZE);
      },
      [&tio, &p_right, F_ndpc_left, c_not_dir_pc](yield_t &yield) {
        mpc_select(tio, yield, p_right, F_ndpc_left, p_right, c_not_dir_pc,
                   AVL_PTR_SIZE);
      },
      [&tio, &ptr_upd, isReal, c_not_dir_pc, p_ptr](yield_t &yield) {
        mpc_select(tio, yield, ptr_upd, isReal, c_not_dir_pc, p_ptr,
                   AVL_PTR_SIZE);
      });

  run_coroutines(
      tio,
      [&tio, &c_left, F_pc_l, ptr_upd](yield_t &yield) {
...
AVL::updateBalanceIns(MPCTIO &tio, yield_t &yield, RegBS bal_l, RegBS bal_r,
  ...
  run_coroutines(
      tio,
      [&tio, &F_rs, child_dir, bal_upd](
          yield_t &yield) { // F_rs (Flag right shift) <- child_dir & bal_upd
        mpc_and(tio, yield, F_rs, child_dir, bal_upd);
      },
      [&tio, &F_ls, nt_child_dir, bal_upd](
          yield_t &yield) { // F_ls (Flag left shift) <- !child_dir & bal_upd
  ...
  coroutines.emplace_back([&tio, &bal_r, F_rs, balanced](yield_t &yield) {
    mpc_select(tio, yield, bal_r, F_rs, bal_r, balanced);
  });
  coroutines.emplace_back([&tio, &balanced, F_rs, bal_l](yield_t &yield) {
    mpc_select(tio, yield, balanced, F_rs, balanced, bal_l);
  });
  coroutines.emplace_back([&tio, &bal_l, F_rs, s0](yield_t &yield) {
  ...
  coroutines.emplace_back([&tio, &imbalance, F_ls, bal_l](yield_t &yield) {
    mpc_select(tio, yield, imbalance, F_ls, imbalance, bal_l);
  });
  coroutines.emplace_back([&tio, &bal_l, F_ls, balanced](yield_t &yield) {
    mpc_select(tio, yield, bal_l, F_ls, bal_l, balanced);
  });
  coroutines.emplace_back([&tio, &balanced, F_ls, bal_r](yield_t &yield) {
    mpc_select(tio, yield, balanced, F_ls, balanced, bal_r);
  });
  coroutines.emplace_back([&tio, &bal_r, F_ls, s0](yield_t &yield) {
...
/*
  ...
  cases are inverted (from that of updateBalanceIns()), since a bal_upd on a
  child implies it reduced height.
...
*/
std::tuple<RegBS, RegBS, RegBS, RegBS>
AVL::updateBalanceDel(MPCTIO &tio, yield_t &yield, RegBS bal_l, RegBS bal_r,
                      RegBS bal_upd, RegBS child_dir) {
  bool player0 = tio.player() == 0;
  RegBS s0;
  ...
  run_coroutines(
      tio,
      [&tio, &F_ls, child_dir, bal_upd](yield_t &yield) {
  ...
  run_coroutines(
      tio,
      [&tio, &imbalance, F_rs, bal_r](yield_t &yield) {
        mpc_select(tio, yield, imbalance, F_rs, imbalance, bal_r);
      },
  ...
  /*
     if(bal_upd) and this node:
      (i) becomes balanced: the height has decreased, so continue propogating
     bal_upd. (ii) becomes imbalanced: fixImbalance will update bal_upd
     correctly. (iii) updates from balanced to left/right heavy: the height of
     this subtree has not changed, so don't propogate bal_upd. We handle (iii)
     below.
  */
  RegBS LR_heavy, bu0;
...
AVL::insert(MPCTIO &tio, yield_t &yield, RegXS ptr, RegXS ins_addr,
            RegAS insert_key, Duoram<Node>::Flat &A, int TTL, RegBS isDummy,
  ...
  */
  ...
  isDummy ^= F_i;
  auto [bal_upd, F_gp, prev_node, prev_dir] = insert(
      tio, yield, next_ptr, ins_addr, insert_key, A, TTL - 1, isDummy, ret);
  /*
  ...
  */
  ...
  auto [new_bal_l, new_bal_r, new_bal_upd, imbalance] =
      updateBalanceIns(tio, yield, bal_l, bal_r, bal_upd, gt);
  ...
  coroutines.emplace_back([&tio, &ret, F_gp, ptr](yield_t &yield) {
    mpc_select(tio, yield, ret.gp_node, F_gp, ret.gp_node, ptr, AVL_PTR_SIZE);
  });
  coroutines.emplace_back([&tio, &ret, F_gp, gt](yield_t &yield) {
    mpc_select(tio, yield, ret.dir_gpp, F_gp, ret.dir_gpp, gt);
  });
  // Save parent pointer
  coroutines.emplace_back([&tio, &ret, imbalance, ptr](yield_t &yield) {
    mpc_select(tio, yield, ret.p_node, imbalance, ret.p_node, ptr,
               AVL_PTR_SIZE);
  });
  coroutines.emplace_back([&tio, &ret, imbalance, gt](yield_t &yield) {
    mpc_select(tio, yield, ret.dir_pc, imbalance, ret.dir_pc, gt);
  });
  // Save child pointer
  coroutines.emplace_back([&tio, &ret, imbalance, prev_node](yield_t &yield) {
    mpc_select(tio, yield, ret.c_node, imbalance, ret.c_node, prev_node,
               AVL_PTR_SIZE);
  });
  coroutines.emplace_back([&tio, &ret, imbalance, prev_dir](yield_t &yield) {
  ...
  run_coroutines(
      tio,
      [&tio, &left, F_il, ins_addr](yield_t &yield) {
...
void AVL::insert(MPCTIO &tio, yield_t &yield, const Node &node) {
  ...
  if (num_items == 0) {
    ...
    auto [bal_upd, F_gp, prev_node, prev_dir] = insert(
        tio, yield, root, insert_address, insert_key, A, TTL, isDummy, ret);
    ...
    */
    ...
    if (OPTIMIZED) {
      ...
      */
      ...
      size_t rec_gp_lptr = mpc_reconstruct(tio, iyield,
      getAVLLeftPtr(A[oidx_gp].NODE_POINTERS), AVL_PTR_SIZE); size_t rec_gp_rptr
      = mpc_reconstruct(tio, yield, getAVLRightPtr(A[oidx_gp].NODE_POINTERS),
      AVL_PTR_SIZE); size_t rec_p_lptr = mpc_reconstruct(tio, yield,
      getAVLLeftPtr(A[oidx_p].NODE_POINTERS), AVL_PTR_SIZE); size_t rec_p_rptr =
      mpc_reconstruct(tio, yield, getAVLRightPtr(A[oidx_p].NODE_POINTERS),
      AVL_PTR_SIZE); size_t rec_c_lptr = mpc_reconstruct(tio, yield,
      getAVLLeftPtr(A[oidx_c].NODE_POINTERS), AVL_PTR_SIZE); size_t rec_c_rptr =
      mpc_reconstruct(tio, yield, getAVLRightPtr(A[oidx_c].NODE_POINTERS),
      AVL_PTR_SIZE); printf("Reconstructed:\ngp_key = %ld, gp_left_ptr = %ld,
    ...
    mpc_select(tio, yield, n_node, ret.dir_cn, child_left, child_right,
               AVL_PTR_SIZE);
    ...
    RegXS old_gp_pointers, old_parent_pointers, old_child_pointers,
        old_n_pointers;
    if (OPTIMIZED) {
    ...
    /* Flags:  F_cn_rot = child->node rotate
       ...
       In case of an imbalance where p->c and c->n are in different directions,
       we have to perform a double rotation (LR or RL case). In such cases,
       first rotate c->n link, and then p->c link (Note: in the second rotation
       c is actually n, since the the first rotation swaps their positions)
    */
    RegBS F_cn_rot, F_ur, s0;
    run_coroutines(
        tio,
        [&tio, &F_ur, F_gp, ret](yield_t &yield) {
    ...
    // Get the n children information for 2nd rotate fix before rotations
    // happen.
    ...
    // If F_cn_rot, i.e. we did first rotation. Then c and n need to swap before
    // the second rotate.
    RegXS new_child_pointers, new_child;

    run_coroutines(
        tio,
        [&tio, &new_child_pointers, F_cn_rot, child_pointers,
         n_pointers](yield_t &yield) {
          mpc_select(tio, yield, new_child_pointers, F_cn_rot, child_pointers,
                     n_pointers);
        },
        [&tio, &new_child, F_cn_rot, ret, n_node](yield_t &yield) {
          mpc_select(tio, yield, new_child, F_cn_rot, ret.c_node, n_node,
                     AVL_PTR_SIZE);
        });
    ...
    run_coroutines(
        tio,
        [&tio, &child_pointers, F_cn_rot, new_child_pointers](yield_t &yield) {
          mpc_select(tio, yield, child_pointers, F_cn_rot, new_child_pointers,
                     child_pointers);
        },
        [&tio, &n_pointers, F_cn_rot, new_child_pointers](yield_t &yield) {
          mpc_select(tio, yield, n_pointers, F_cn_rot, n_pointers,
                     new_child_pointers);
        });
    ...
    run_coroutines(
        tio,
        [&tio, &c_bal_l, ret, s0](yield_t &yield) {
    ...
    run_coroutines(
        tio,
        [&tio, &n_l0, n_l, &aes_ops](yield_t &yield) {
    ...
    run_coroutines(
        tio,
        [&tio, &p_c_update, F_cn_rot, n_has_children](yield_t &yield) {
    ...
    run_coroutines(
        tio,
        [&tio, &p_bal_ndpc, p_c_update, n_bal_ndpc](yield_t &yield) {
          mpc_select(tio, yield, p_bal_ndpc, p_c_update, p_bal_ndpc,
                     n_bal_ndpc);
        },
    ...
    // In all other cases, n's balance remains unaffected by rotation during
    // insertion.
    ...
    if (OPTIMIZED) {
      run_coroutines(
          tio,
          [&tio, &A, &oidx_n, n_pointers, old_n_pointers](yield_t &yield) {
            auto Acont = A.context(yield);
            Acont[oidx_n.value()].NODE_POINTERS +=
                (n_pointers - old_n_pointers);
          },
          [&tio, &A, &oidx_c, child_pointers,
           old_child_pointers](yield_t &yield) {
            auto Acont = A.context(yield);
            Acont[oidx_c.value()].NODE_POINTERS +=
                (child_pointers - old_child_pointers);
          },
          [&tio, &A, &oidx_p, parent_pointers,
           old_parent_pointers](yield_t &yield) {
            auto Acont = A.context(yield);
            Acont[oidx_p.value()].NODE_POINTERS +=
                (parent_pointers - old_parent_pointers);
          },
          [&tio, &A, &oidx_gp, gp_pointers, old_gp_pointers](yield_t &yield) {
            auto Acont = A.context(yield);
            Acont[oidx_gp.value()].NODE_POINTERS +=
                (gp_pointers - old_gp_pointers);
          });
    } else {
    ...
    run_coroutines(
        tio,
        [&tio, &temp_root, F_ur, ret](yield_t &yield) {
          mpc_select(tio, yield, temp_root, F_ur, temp_root, ret.c_node,
                     AVL_PTR_SIZE);
        },
...
bool AVL::lookup(MPCTIO &tio, yield_t &yield, RegXS ptr, RegAS key,
                 Duoram<Node>::Flat &A, int TTL, RegBS isDummy,
                 Node *ret_node) {
  if (TTL == 0) {
  ...
  RegBS F_found;
  // If we haven't found the key yet, and the lookup matches the current node
  // key, then we found the node to return

  // If multiple keys in the tree match the lookup key, this returns the last
  // match. Extracting the first match would add an extra round here, since the
  // F_found flag will have to be computed first, then the next two based on
  // F_found instead of eq
  run_coroutines(
      tio,
      ...
      [&tio, &ret_node, F_found, &cnode](yield_t &yield) {
        mpc_select(tio, yield, ret_node->key, F_found, ret_node->key,
                   cnode.key);
      },
      [&tio, &ret_node, F_found, &cnode](yield_t &yield) {
        mpc_select(tio, yield, ret_node->value, F_found, ret_node->value,
                   cnode.value);
      });
...
void AVL::updateChildPointers(MPCTIO &tio, yield_t &yield, RegXS &left,
  ...
  run_coroutines(
      tio,
      [&tio, &F_rr, c_prime, ret_struct](yield_t &yield) {
  ...
  run_coroutines(
      tio,
      [&tio, &right, F_rr, ret_struct](yield_t &yield) {
...
/*
   For capturing both the symmetric L and R cases of rotations, we'll capture
   directions with dpc  = dir_pc = direction from parent to child, and ndpc =
   not(dir_pc) When we travelled down the stack, we went from p->c. But in
   deletions to handle any imbalance we look at c's sibling cs (child's
   sibling). And the rotation is between p and cs if there was an imbalance at
   p, and perhaps even cs and its child (the child in dir_pc, as that's the only
   case that results in a double rotation when deleting).
   ...
   In such cases, first rotate cs->gcs link, and then p->cs link. gcs =
   grandchild on cs path
...
void AVL::fixImbalance(MPCTIO &tio, yield_t &yield, Duoram<Node>::Flat &A,
                       Duoram<Node>::OblivIndex<RegXS, 1> oidx,
                       RegXS oidx_oldptrs, RegXS ptr, RegXS nodeptrs,
                       RegBS new_p_bal_l, RegBS new_p_bal_r, RegBS &bal_upd,
  ...
  run_coroutines(
      tio,
      [&tio, &F_dr, imb, cs_bal_dpc](yield_t &yield) {
        mpc_and(tio, yield, F_dr, imb, cs_bal_dpc);
      },
      [&tio, &gcs_ptr, cs_bal_dpc, cs_ndpc, cs_dpc](yield_t &yield) {
        mpc_select(tio, yield, gcs_ptr, cs_bal_dpc, cs_ndpc, cs_dpc,
                   AVL_PTR_SIZE);
      });
  ...
  run_coroutines(
      tio,
      [&tio, &gcs_bal_dpc, dpc_is_r, gcs_bal_l, gcs_bal_r](yield_t &yield) {
  ...
  // If F_dr, we did first rotation. Then cs and gcs need to swap before the
  // second rotate.
  RegXS new_cs_pointers, new_cs, new_ptr;
  run_coroutines(
      tio,
      [&tio, &new_cs_pointers, F_dr, cs_node, gcs_node](yield_t &yield) {
        mpc_select(tio, yield, new_cs_pointers, F_dr, cs_node.pointers,
                   gcs_node.pointers);
      },
  ...
  rotate(tio, yield, null, ptr, nodeptrs, new_cs, new_cs_pointers, s0,
         not_c_prime, imb, s1);
  ...
  coroutines.emplace_back([&tio, &ret_struct, imb, new_ptr](yield_t &yield) {
    mpc_select(tio, yield, ret_struct.ret_ptr, imb, ret_struct.ret_ptr,
               new_ptr);
  });
  // Write back new_cs_pointers correctly to (cs_node/gcs_node).pointers
  // and then balance the nodes
  coroutines.emplace_back(
      [&tio, &cs_node, F_dr, new_cs_pointers](yield_t &yield) {
        mpc_select(tio, yield, cs_node.pointers, F_dr, new_cs_pointers,
                   cs_node.pointers);
      });
  coroutines.emplace_back(
      [&tio, &gcs_node, F_dr, new_cs_pointers](yield_t &yield) {
        mpc_select(tio, yield, gcs_node.pointers, F_dr, gcs_node.pointers,
                   new_cs_pointers);
      });
  run_coroutines(tio, coroutines);
  coroutines.clear();
  ...
  /*
     ...
     In the case of an imbalance, updateBalance() sets bal_l and bal_r of p to
     0.
  */
  ...
  run_coroutines(
      tio,
      [&tio, &IC1, imb, cs_bal_ndpc](yield_t &yield) {
      ...
      [&tio, &IC3, imb, cs_bal_dpc](yield_t &yield) {
        // IC3 = Double rotation (LR/RL). 1st rotate direction = ndpc, 2nd
        // direction = dpc
  ...
  */
  ...
  coroutines.emplace_back(
      [&tio, &gcs_bal_r, dpc_is_r, gcs_bal_dpc](yield_t &yield) {
  ...
  // Updating cs_bal_l/r (cs_bal_dpc effected by IC3, cs_bal_ndpc effected by
  // IC1,2)
  coroutines.emplace_back(
      [&tio, &cs_bal_r, dpc_is_r, cs_bal_dpc](yield_t &yield) {
  ...
  coroutines.emplace_back(
      [&tio, &new_p_bal_r, ndpc_is_r, p_bal_ndpc](yield_t &yield) {
  ...
  coroutines.emplace_back(
      [&tio, &cs_bal_r, ndpc_is_r, cs_bal_ndpc](yield_t &yield) {
  ...
  if (OPTIMIZED) {
    coroutines.emplace_back([&tio, &A, &oidx_cs, &cs_node,
                             old_cs_ptr](yield_t &yield) {
      auto acont = A.context(yield);
    ...
    coroutines.emplace_back(
        [&tio, &A, &oidx_gcs, &gcs_node, old_gcs_ptr](yield_t &yield) {
          auto acont = A.context(yield);
          (acont[oidx_gcs.value()].NODE_POINTERS) +=
              (gcs_node.pointers - old_gcs_ptr);
        });
    coroutines.emplace_back(
...
/*
    ...
    F_sf = successor found (no more left children while trying to find
   successor) F_r  = Flag for updating with ret_struct.ret_ptr. F_r happens in 3
   cases. It's subflag F_rs, handles cases (i) and (ii).
...
void AVL::updateRetStruct(MPCTIO &tio, yield_t &yield, RegXS ptr, RegBS F_rs,
  ...
  run_coroutines(
      tio,
      [&tio, &ret_struct, F_nr, ptr](yield_t &yield) {
        mpc_select(tio, yield, ret_struct.ret_ptr, F_nr, ret_struct.ret_ptr,
                   ptr);
      },
      [&tio, &bal_upd, F_rs,
       s1](yield_t
               &yield) { // If F_rs, we skipped a node, so update bal_upd to 1
        mpc_select(tio, yield, bal_upd, F_rs, bal_upd, s1);
      });
}

std::tuple<bool, RegBS> AVL::del(MPCTIO &tio, yield_t &yield, RegXS ptr,
                                 RegAS del_key, Duoram<Node>::Flat &A,
                                 RegBS found, RegBS find_successor, int TTL,
                                 avl_del_return &ret_struct) {
  bool player0 = tio.player() == 0;
  if (TTL == 0) {
    ...
    Node node;
    RegXS oldptrs;
    // This OblivIndex creation is not required if we are not running optimized
    // version, but for convenience we leave it in, so that fixImbalance has an
    // oidx to be supplied when we are in the non-optimized setting.
    nbits_t width = ceil(log2(cur_max_index + 1));
    typename Duoram<Node>::template OblivIndex<RegXS, 1> oidx(tio, yield, ptr,
                                                              width);
    if (OPTIMIZED) {
      node = A[oidx];
    ...
    run_coroutines(
        tio,
        [&tio, &l0, left, &aes_ops](yield_t &yield) {
          CDPF cdpf = tio.cdpf(yield);
        ...
        [&tio, &lt, &eq, &gt, del_key, node, aes_ops](yield_t &yield) {
          CDPF cdpf = tio.cdpf(yield);
          auto [a, b, c] =
              cdpf.compare(tio, yield, del_key - node.key, tio.aes_ops());
          lt = a;
          eq = b;
          gt = c;
        });
    ...
    // In find successor case, so we find inorder successor for node to be
    // deleted (inorder successor = go right and then find leftmost child.)

    // Case 3: finding successor (find_successor) and node has both children
    // (F_2) Go left.
    ...
    // Case 4: finding successor (find_successor) and node has no more left
    // children (l0) This is the successor node then. Go right (since no more
    // left)
    run_coroutines(
        tio,
    ...
    run_coroutines(
        tio,
        [&tio, &c_prime, F_c4, l0](yield_t &yield) {
    ...
    run_coroutines(
        tio,
        [&tio, &cs_ptr, c_prime, right, left](yield_t &yield) {
    ...
    TTL -= 1;
    auto [key_found, bal_upd] =
        del(tio, yield, next_ptr, del_key, A, found_prime, find_successor_prime,
            TTL, ret_struct);
    ...
    RegBS F_ri;
    fixImbalance(tio, yield, A, oidx, oldptrs, ptr, node.pointers, new_p_bal_l,
                 new_p_bal_r, bal_upd, c_prime, cs_ptr, imb, F_ri, ret_struct);
...
bool AVL::del(MPCTIO &tio, yield_t &yield, RegAS del_key) {
  ...
  if (num_items == 1) {
    ...
    Node zero;
    nbits_t width = ceil(log2(cur_max_index + 1));
    typename Duoram<Node>::template OblivIndex<RegXS, 1> oidx(tio, yield, root,
                                                              width);
    Node node = A[oidx];
    // Compare key
    CDPF cdpf = tio.cdpf(yield);
    ...
    int TTL = AVL_TTL(num_items);
    // Flags for already found (found) item to delete and find successor
    // (find_successor) if this deletion requires a successor swap
    RegBS found, find_successor;
    avl_del_return ret_struct;
    auto [success, bal_upd] = del(tio, yield, root, del_key, A, found,
                                  find_successor, TTL, ret_struct);
    // printf ("Success =  %d\n", success);
    if (!success) {
      return false;
    } else {
      ...
      */
      ...
      run_coroutines(
          ...
          [&tio, &del_node, ret_struct, suc_node](yield_t &yield) {
            mpc_select(tio, yield, del_node.key, ret_struct.F_ss, del_node.key,
                       suc_node.key);
          },
          [&tio, &del_node, ret_struct, suc_node](yield_t &yield) {
            mpc_select(tio, yield, del_node.value, ret_struct.F_ss,
                       del_node.value, suc_node.value);
          },
          [&tio, &empty_loc, ret_struct](yield_t &yield) {
            mpc_select(tio, yield, empty_loc, ret_struct.F_ss, ret_struct.N_d,
                       ret_struct.N_s);
          });
      ...
      if (OPTIMIZED) {
        ...
        coroutines.emplace_back([&tio, &A, &oidx_nd, &del_node,
                                 old_del_value](yield_t &yield) {
          auto acont = A.context(yield);
      ...
      // Add deleted (empty) location into the empty_locations vector for reuse
      // in next insert()
...
void avl(MPCIO &mpcio, const PRACOptions &opts, char **args) {
  ...
  /* The ORAM will be initialized with 2^depth-1 items, but the 0 slot is
     reserved. So we initialize (initial inserts) with 2^depth-2 items. The ORAM
     size is set to 2^depth-1 + n_insert.
  */
  size_t init_size = (size_t(1) << (depth));
  size_t oram_size = init_size + n_inserts;

  MPCTIO tio(mpcio, 0, opts.num_cpu_threads);
  run_coroutines(tio, [&tio, &mpcio, depth, oram_size, init_size, n_inserts,
                       n_deletes, run_sanity, optimized](yield_t &yield) {

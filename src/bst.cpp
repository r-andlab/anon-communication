...
std::tuple<bool, address_t> BST::check_bst(const std::vector<Node> &R,
                                           value_t node, value_t min_key = 0,
                                           value_t max_key = ~0) {
  // printf("node = %ld\n", node);
  if (node == 0) {
    return {true, 0};
  ...
  return {leftok && rightok && key >= min_key && key <= max_key, height};
}
...
std::tuple<RegXS, RegBS> BST::insert(MPCTIO &tio, yield_t &yield, RegXS ptr,
                                     RegAS insertion_key, Duoram<Node>::Flat &A,
                                     int TTL, RegBS isDummy) {
  if (TTL == 0) {
    RegBS zero;
    return {ptr, zero};
  }
  ...
  isDummy ^= F_i;
  auto [wptr, direction] =
      insert(tio, yield, next_ptr, insertion_key, A, TTL - 1, isDummy);
  ...
  run_coroutines(
      tio,
      [&tio, &ret_ptr, F_i, wptr, ptr](yield_t &yield) {
...
*/
void BST::insert(MPCTIO &tio, yield_t &yield, const Node &node,
                 Duoram<Node>::Flat &A) {
  ...
  if (num_items == 0) {
    ...
    auto [wptr, direction] =
        insert(tio, yield, root, node.key, A, TTL, isDummy);
    ...
    run_coroutines(
        tio,
        [&tio, &new_right_ptr, direction, right_ptr,
         insert_address](yield_t &yield) {
          mpc_select(tio, yield, new_right_ptr, direction, right_ptr,
                     insert_address);
        },
        [&tio, &new_left_ptr, not_direction, left_ptr,
         insert_address](yield_t &yield) {
          mpc_select(tio, yield, new_left_ptr, not_direction, left_ptr,
                     insert_address);
        });
...
RegBS BST::lookup(MPCTIO &tio, yield_t &yield, RegXS ptr, RegAS key,
  ...
  coroutines.emplace_back([&tio, &next_ptr, gt, left, right](yield_t &yield) {
    mpc_select(tio, yield, next_ptr, gt, left, right, 32);
  });
  coroutines.emplace_back([&tio, &F_found, isNotDummy, eq](yield_t &yield) {
    mpc_and(tio, yield, F_found, isNotDummy, eq);
  });
  coroutines.emplace_back([&tio, &ret_node, eq, cnode](yield_t &yield) {
  ...
  coroutines.emplace_back([&tio, &isDummy, eq](yield_t &yield) {
  ...
  printf("rec_isDummy/found = %d ,rec_f_found = %d, cnode.key = %ld, lookup "
         "key = %ld\n",
         rec_found, rec_f_found, ckey, lkey);
#endif

  RegBS found =
      lookup(tio, yield, next_ptr, key, A, TTL - 1, isDummy, ret_node);
  ...
  if (TTL == 0) {
    ...
    */
    ...
    run_coroutines(
        ...
        // Case 4: finding successor (fs) and node has no more left children
        // (l0) This is the successor node then. Go right (since no more left)
    ...
    bool key_found = del(tio, yield, next_ptr, del_key, A, af_prime, fs_prime,
                         TTL - 1, ret_struct);
    ...
    // Flag here should be direction (c_prime) and F_r i.e. we need to swap
    // return ptr in, F_r needs to be returned in ret_struct
    run_coroutines(
        tio,
    ...
    */
    ...
    run_coroutines(
        ...
        [&tio, &ret_struct, F_r, ptr](yield_t &yield) {
          mpc_select(tio, yield, ret_struct.ret_ptr, F_r, ptr,
                     ret_struct.ret_ptr);
        });
      ...
      */
      ...
      run_coroutines(
          ...
          [&tio, &temp_root, ret_struct](yield_t &yield) {
            mpc_select(tio, yield, temp_root, ret_struct.F_r, temp_root,
                       ret_struct.ret_ptr);
          },
...
// online.cpp.
void bst(MPCIO &mpcio, const PRACOptions &opts, char **args) {
  nbits_t depth = 4;

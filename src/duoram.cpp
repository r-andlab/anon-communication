...
template <> RegAS Duoram<RegAS>::Shape::basic_binary_search(RegAS &target) {
  ...
  while (depth > 0) {
    ...
    auto [lt, eq, gt] =
        cdpf.compare(this->tio, this->yield, val - target, tio.aes_ops());
    if (depth > 1) {
...
template <> RegXS Duoram<RegAS>::Shape::binary_search(RegAS &target) {
  if (this->shape_size == 0) {
    RegXS zero;
    return zero;
  }

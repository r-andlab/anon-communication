...
void mpc_mul(MPCTIO &tio, yield_t &yield, RegAS &z, RegAS x, RegAS y,
             nbits_t nbits) {
...
// 2 words sent in 1 message
// consumes 1 MultTriple
void mpc_cross(MPCTIO &tio, yield_t &yield, RegAS &z, RegAS x, RegAS y,
               nbits_t nbits) {
  const value_t mask = MASKBITS(nbits);
...
void mpc_valuemul(MPCTIO &tio, yield_t &yield, RegAS &z, value_t x,
...
// 2 words sent in 1 message
// consumes 1 MultTriple
void mpc_flagmult(MPCTIO &tio, yield_t &yield, RegAS &z, RegBS f, RegAS y,
                  nbits_t nbits) {
  const value_t mask = MASKBITS(nbits);
...
// 2 words sent in 1 message
// consumes 1 MultTriple
void mpc_select(MPCTIO &tio, yield_t &yield, RegAS &z, RegBS f, RegAS x,
                RegAS y, nbits_t nbits) {
  const value_t mask = MASKBITS(nbits);
...
void mpc_select(MPCTIO &tio, yield_t &yield, RegXS &z, RegBS f, RegXS x,
                RegXS y, nbits_t nbits) {
  const value_t mask = MASKBITS(nbits);
  size_t nbytes = BITBYTES(nbits);
  ...
  z.xshare = ((fext & peer_blind_d) ^ (Y & peer_blind_fext) ^ (fext & d) ^
              (Z ^ x.xshare)) &
             mask;
}
...
void mpc_select(MPCTIO &tio, yield_t &yield, RegBS &z, RegBS f, RegBS x,
                RegBS y) {
  // The desired result is z = x ^ (f & (y^x))
...
// 2 words sent in 1 message
// consumes 1 MultTriple
void mpc_oswap(MPCTIO &tio, yield_t &yield, RegAS &x, RegAS &y, RegBS f,
               nbits_t nbits) {
  const value_t mask = MASKBITS(nbits);
...
void mpc_xs_to_as(MPCTIO &tio, yield_t &yield, RegAS &as_x, RegXS xs_x,
                  nbits_t nbits, bool tally) {
  const value_t mask = MASKBITS(nbits);
...
value_t mpc_reconstruct(MPCTIO &tio, yield_t &yield, RegXS x, nbits_t nbits) {
  RegXS res;
  size_t nbytes = BITBYTES(nbits);
  if (tio.player() < 2) {
...
//
// Cost: 1 word sent in 1 message
value_t mpc_reconstruct(MPCTIO &tio, yield_t &yield, RegAS x, nbits_t nbits) {
  RegAS res;
  size_t nbytes = BITBYTES(nbits);
  if (tio.player() < 2) {
    tio.queue_peer(&x, nbytes);
...
void mpc_reconstruct_choice(MPCTIO &tio, yield_t &yield, DPFnode &z, RegBS f,
                            DPFnode x, DPFnode y) {
  // Sign-extend f (so 0 -> 0000...0; 1 -> 1111...1)
  ...
  DPFnode zshare =
      (fext & peer_blind_d) ^ (Y & peer_blind_fext) ^ (fext & d) ^ (Z ^ x);
...
// 1 byte sent in 1 message
// consumes 1/64 AndTriple
void mpc_and(MPCTIO &tio, yield_t &yield, RegBS &z, RegBS x, RegBS y) {
...
// 1 byte sent in 1 message
// consumes 1/64 AndTriple
void mpc_or(MPCTIO &tio, yield_t &yield, RegBS &z, RegBS x, RegBS y) {
  if (tio.player() == 0) {
    x.bshare = !x.bshare;
    y.bshare = !y.bshare;
  }

...
void RemiseSAM::post_message(MPCTIO &tio, yield_t &yield, RegAS ind, RegAS val,
                             RegAS proof) {
  ...
  run_coroutines(
      tio,
      [&cdpf, &tio, &yield, &proof, &ProofShare, &lt_audit, &eq_audit,
       &gt_audit](yield_t &inner_yield) {
        std::tie(lt_audit, eq_audit, gt_audit) =
            cdpf.compare(tio, inner_yield, proof - ProofShare, tio.aes_ops());
      },
      [&cdpf, &tio, &yield, &val, &zero_share, &lt_zero_msg, &eq_zero_msg,
       &gt_zero_msg](yield_t &inner_yield) {
        std::tie(lt_zero_msg, eq_zero_msg, gt_zero_msg) =
            cdpf.compare(tio, inner_yield, val - zero_share, tio.aes_ops());
      });
  ...
  bool final_audit_check_reconstruction =
      mpc_reconstruct(tio, yield, final_audit_check);

#ifdef VERBOSE
  std::cout << "final_audit_check_reconstruction = "
            << (int)final_audit_check_reconstruction << std::endl;
#endif
...
RegAS RemiseSAM::read_message(MPCTIO &tio, yield_t &yield, RegAS ind,
                              RegAS proof_submitted) {
  ...
  auto [lt_read_auth, eq_read_auth, gt_read_auth] =
      cdpf.compare(tio, yield, proof_share - proof_submitted, tio.aes_ops());
...
void RemiseSAM::provide_or_remove_access(MPCTIO &tio, yield_t &yield, RegAS ind,
                                         RegAS proof_submitted) {
  ...
  auto [lt_audit, eq_audit, gt_audit] = cdpf.compare(
      tio, yield, proof_submitted - ProofShare,
      tio.aes_ops()); // Checks if the message submitted is a zero message
...
void RemiseSAM::initialize_proofDB(MPCTIO &tio, yield_t &yield, size_t dbsize,
                                   Writer writer) {
...
void RemiseSAM::publish_bulletin_board(MPCTIO &tio, yield_t &yield,
                                       size_t dbsize) {
...
void Remise(MPCIO &mpcio, const PRACOptions &opts, char **args) {
  ...
  run_coroutines(tio, [&tio, dbsize, n_correct_posts, n_failed_posts,
                       n_cover_traffic, n_provide_access, n_reads,
                       &mpcio](yield_t &yield) {
    size_t size = size_t(1) << dbsize;

    RemiseSAM bulletinboard(tio.player(), size);
    ...
    // std::cout << "index_to_which_posted = " << index_to_which_posted <<
    // std::endl;

    for (size_t j = 0; j < n_correct_posts; ++j) {
      if (tio.player() == 0)
        proof_submitted.ashare = 0;
      if (tio.player() == 1)
        proof_submitted.ashare = writer.proofs[j];
      ...
      bulletinboard.post_message(tio, yield, ind, inserted_val,
                                 proof_submitted);
    ...
    proof_submitted.randomize();

    for (size_t j = 0; j < n_provide_access; ++j) {
      bulletinboard.provide_or_remove_access(tio, yield, ind, proof_submitted);
    }

    std::cout << "\n\n\n\n\n\n===== Providing Access Stats =====\n";
    std::cout << "n_provide_access = " << n_provide_access << std::endl;
    tio.sync_lamport();
    mpcio.dump_stats(std::cout);
    mpcio.reset_stats();
    tio.reset_lamport();

...
static void usage(const char *progname) {
  std::cerr << "Usage: " << progname
            << " [-p | -a | -o] [-C num] [-t num] [-e] [-x] player_num "
               "player_addrs args ...\n";
  ...
  std::cerr << "player_addrs is p0's hostname followed by p1's hostname for "
               "player 2\n";
  exit(1);
}

static void comp_player_main(boost::asio::io_context &io_context,
                             unsigned player, const PRACOptions &opts,
                             const char *p0addr, char **args) {
  std::deque<tcp::socket> peersocks, serversocks;
  mpcio_setup_computational(player, io_context, p0addr, opts.num_comm_threads,
...
static void server_player_main(boost::asio::io_context &io_context,
  ...
  mpcio_setup_server(io_context, p0addr, p1addr, opts.num_comm_threads, p0socks,
                     p1socks);
